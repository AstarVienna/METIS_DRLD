...
\begin{recipedef}\label{rec:metis_img_adi_cgrph}
  Name:                & \NEWREC{metis_img_adi_cgrph}                                        \\
  Purpose:             & Classical ADI post processing for CVC/RAVC coronagraphs      \\
  Requirements:        & \REQ{METIS-5989}                                               \\
  Templates:           & None                               \\
  Type:                & Science                                                    \\
  Input data:          & \NEWPROD{LM_SCI_BASIC_REDUCED} or \NEWPROD{N_SCI_BKG_SUBTRACTED} \\
% TODO: There used to be a reference to N_SCI_BASIC_REDUCED here, but that is not defined, should it be added?
% or   \NEWPROD{N_SCI_BASIC_REDUCED}                         \\
% now replaced with N_SCI_BKG_SUBTRACTE
                       & \NEWPROD{LM_DISTORTION_TABLE} or \NEWPROD{N_DISTORTION_TABLE} \\
%                        & Coronagraphic throughput map and profile         \\
                       & \NEWPROD{LM_cgrph_SCI_THROUGHPUT} or \NEWPROD{N_cgrph_SCI_THROUGHPUT} \\
                       & \NEWRAW{LM_OFF_AXIS_PSF_RAW} or \NEWRAW{N_OFF_AXIS_PSF_RAW}\\
  ...
  Output data:       & \NEWPROD{LM_cgrph_SCI_CALIBRATED} or \NEWPROD{N_cgrph_SCI_CALIBRATED}\\
                     & \NEWPROD{LM_cgrph_SCI_CENTRED} or \NEWPROD{N_cgrph_SCI_CENTRED}\\
                     & \NEWPROD{LM_cgrph_CENTROID_TAB} or \NEWPROD{N_cgrph_CENTROID_TAB}\\
                     & \NEWPROD{LM_cgrph_SCI_SPECKLE} or \NEWPROD{N_cgrph_SCI_SPECKLE}\\
                     & \NEWPROD{LM_cgrph_SCI_DEROTATED_PSFSUB} or \NEWPROD{N_cgrph_SCI_DEROTATED_PSFSUB}\\
                     & \NEWPROD{LM_cgrph_SCI_DEROTATED} or \NEWPROD{N_cgrph_SCI_DEROTATED}\\
                     & \NEWPROD{LM_cgrph_SCI_CONTRAST_RADPROF} or \NEWPROD{N_cgrph_SCI_CONTRAST_RADPROF}\\
                     & \NEWPROD{LM_cgrph_SCI_CONTRAST_ADI} or \NEWPROD{N_cgrph_SCI_CONTRAST_ADI}\\
                     & \NEWPROD{LM_cgrph_SCI_THROUGHPUT} or \NEWPROD{N_cgrph_SCI_THROUGHPUT}\\
                     & \NEWPROD{LM_cgrph_SCI_COVERAGE} or \NEWPROD{N_cgrph_SCI_COVERAGE}\\
                     & \NEWPROD{LM_cgrph_SCI_SNR} or \NEWPROD{N_cgrph_SCI_SNR}\\
                     & \NEWPROD{LM_cgrph_PSF_MEDIAN} org \NEWPROD{N_cgrph_PSF_MEDIAN} \\
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=0.9\textwidth]{./figures/metis_lm_adi_ravc}
  \caption[Recipe: \REC{metis_img_adi_cgrph}]{\NEWREC{metis_img_adi_cgrph} -- DET ADI post processing for RAVC/CVC coronagraph.
    }
...
%                        & call \hyperref[drl:lm_adi_app_psf]{\CODE{lm_adi_app_psf}} or \hyperref[drl:n_adi_app_psf]{\CODE{n_adi_app_psf}} to determine the median PSF \\
  ...
  Output data:       & \NEWPROD{LM_APP_SCI_CALIBRATED}\\
                     & \NEWPROD{LM_APP_SCI_CENTRED}\\
                     & \NEWPROD{LM_APP_CENTROID_TAB}\\
                     & \NEWPROD{LM_APP_SCI_SPECKLE}\\
                     & \NEWPROD{LM_APP_SCI_DEROTATED_PSFSUB}\\
                     & \NEWPROD{LM_APP_SCI_DEROTATED}\\
                     & \NEWPROD{LM_APP_SCI_CONTRAST_RADPROF}\\
                     & \NEWPROD{LM_APP_SCI_CONTRAST_ADI}\\
                     & \NEWPROD{LM_APP_SCI_THROUGHPUT}\\
                     & \NEWPROD{LM_APP_SCI_COVERAGE}\\
                     & \NEWPROD{LM_APP_SCI_SNR}\\
                     & \NEWPROD{LM_APP_PSF_MEDIAN}\\
Expected accuracies: & n/a                                                           \\
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=0.9\textwidth]{./figures/metis_lm_adi_app}
...
\begin{recipedef}
  Name:                & \NEWREC{metis_ifu_adi_cgrph}\label{rec:metis_ifu_adi_cgrph}                                        \\
...
%   Input data:          & \NEWPROD{IFU_cgrph_SCI_REDUCED}                            \\
  Input data:          & \NEWPROD{IFU_SCI_REDUCED}                            \\
                       & \NEWPROD{IFU_DISTORTION_TABLE}\\
%                        & Coronagraphic throughput map and profile                                                  \\
                       & \NEWPROD{IFU_cgrph_SCI_THROUGHPUT} \\
                       & \NEWRAW{IFU_OFF_AXIS_PSF_RAW} \\
   Matched keywords:   & Detector ID             \\
                       & Filter ID               \\
                       & Object ID               \\
  Parameters:          & combination method (median, mean, sigclip, \dots)\\
                       ...
                       & start and end limit to contrast curve (in $\lambda/D$) \\
                       & frame exclusion thresholds dependent on AO parameters and centroid offset \\
  Algorithm:           & call \hyperref[drl:lm_adi_cgrph_centroid]{\CODE{det_adi_cgrph_centroid}} to determine the location of the central PSFs \\
  ...
  Output data:       & \NEWPROD{IFU_cgrph_SCI_CALIBRATED}\\
                     & \NEWPROD{IFU_cgrph_SCI_CENTRED}\\
                     & \NEWPROD{IFU_cgrph_CENTROID_TAB}\\
                     & \NEWPROD{IFU_cgrph_SCI_SPECKLE}\\
                     & \NEWPROD{IFU_cgrph_SCI_DEROTATED_PSFSUB}\\
                     & \NEWPROD{IFU_cgrph_SCI_DEROTATED}\\
                     & \NEWPROD{IFU_cgrph_SCI_CONTRAST_RADPROF}\\
                     & \NEWPROD{IFU_cgrph_SCI_CONTRAST_ADI}\\
                     & \NEWPROD{IFU_cgrph_SCI_THROUGHPUT}\\
                     & \NEWPROD{IFU_cgrph_SCI_SNR}\\
                     & \NEWPROD{IFU_cgrph_SCI_COVERAGE}                           \\
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=0.9\textwidth]{./figures/metis_ifu_adi_ravc}
