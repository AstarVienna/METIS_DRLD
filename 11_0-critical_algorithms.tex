...
In case a persistence correction is required, a subtraction of a persistence map (\NEWEXTCALIB{PERSISTENCE_MAP}) is foreseen in any individual recipe. This map is created by the recipe \NEWREC{metis_det_persistence} (cf. Section~\ref{rec:metis_det_persistence}) , which is carried out by \ac{ESO} and relies on the algorithm provided by \ac{ESO}.
...
%    \NEWSTATCALIB{ATM_LINE_CAT}) is possible.
%\end{itemize}
