...
% TODO: We are considering including a persistence correction step in the flat
% and dark recipes anyway, as the code should not actually perform any
% correction if it detect that no persistence is present and that correcting
% it would only add noise. Nevertheless, for now we keep that out of the DRLD
% to close our FDR Action Items with the least effort. We can add that
% mostly redundant persistence correction step later if we desire.
% See https://github.com/AstarVienna/METIS_DRLD/issues/229

Calibration data where the flux is controlled (e.g. dark and flat) should not
be affected by persistence. No persistence correction step is therefore
included in those recipes.
...
The algorithm is implemented in \REC{metis_ifu_reduce}.
