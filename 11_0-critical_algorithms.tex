...
In case a persistence correction is required, a subtraction of a persistence map () is foreseen in any individual recipe. This map is created by the recipe \REC{metis_det_persistence} (cf. Section~\label{rec:metis_det_persistence}) , which is carried out by \ac{ESO} and relies on the algorithm provided by \ac{ESO}.
