...
Alternatively, the reconstruction algorithm can operate directly on unrectified detector data,
producing the entire model cube in one pass, at the cost of having to work with significantly larger data and model vectors.
This has an advantage of eliminating the first resampling, thus avoiding the need for the covariance matrix altogether.
In this case the size of the data vector is $6 \times 4096 \times 4096 \approx 10^8$
and the size of the model vector increases to $68 \times 68 \times 4096 \approx 2 \cdot 10^7$.
The size of $\mat{A}$ is then enormous -- however as each detector pixel contributes
to at most a small constant number of output voxels, it is also extremely sparse.

Furthermore, as long as the distortion and the angle between the data pixels and across-slice direction is sufficiently small,
it is possible to process the detector data in layers in the $\lambda$ direction separately
with negligible loss of precision, as shown by \cite{Law+2023} for the MIRI instrument at JWST.
The exact position of each detector pixel is translated directly to the output cube space,
affecting at most a small constant number of layers in the $\lambda$ direction.

\subsubsection{Precision of positions}
    The described algorithm does not assume that exact positions of exposures are equidistant
    and the differences in orientation of the exposure sets are exactly 90 degrees.
    However, it assumes that the positions and rotations of the individual exposures
    are \emph{known} with sufficient precision and stored in the corresponding FITS headers.

    If this is not the case, repeated runs of the entire algorithm are required,
    with relative positions and rotations of individual exposures varied by small values
    on the order of a fraction of pixel size.
    The solution for which the $\chi^2$ error (see equation \ref{eq:chi2_minimisation})
    is minimal is then declared to be the best estimate of the original model image.
