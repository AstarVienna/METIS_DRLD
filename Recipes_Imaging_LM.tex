...
\begin{recipedef}
  ...
  Input data:          & \NEWRAW{LM_FLAT_LAMP_RAW} \\
                       & or \NEWRAW{LM_FLAT_TWILIGHT_RAW} \\
                       & \NEWPROD{MASTER_DARK_2RG} (for twilight flats) \\
...
%                       & possibly others (e.g.\ coronagraphic mask, \TBD)               \\
 ...
 Algorithm:            & Call \NEWDRL{metis_apply_persistence_correction} to apply the persistence correction \\
                         & For internal flats: call \NEWREC{metis_det_dark} with \CODE{LAMP OFF} images to create dark frame. \\
 & Subtract internal dark or master dark from flat exposures.     \\
  & call \NEWREC{metis_lm_img_flat} to fit slope of pixel values against illumination level. Frames
  with the same exposure time will be averaged.\\
                       & Compute median or average of input frames to improve statistics.\\
                       & Call \NEWDRL{metis_update_lm_flat_mask} to flag deviant pixels. \\
  Output data:         & \NEWPROD{MASTER_IMG_FLAT_LAMP_LM} \\
                       & or \NEWPROD{MASTER_IMG_FLAT_TWILIGHT_LM} \\
                       & \NEWPROD{BADPIX_MAP_2RG}                                           \\
...
\begin{recipedef}
  Name:             & \NEWREC{metis_lm_img_basic_reduce}   \\
...
%                     & \TPL{METIS_img_lm_*_obs_*}       \\
  ...
  Input data:       & \NEWRAW{LM_IMAGE_SCI_RAW} (Dithered images (standard, science) \\
%                     & \NEWRAW{IFU_SKY_RAW} (Blank sky images, if available.) \\
%                     & \NEWEXTCALIB{LM_DETECTOR_MASK} (if available)  \\
  ...
  Output data:      & \NEWPROD{LM_SCI_BASIC_REDUCED}       \\
...
\begin{recipedef}
  Name:             & \NEWREC{metis_lm_img_background}                             \\
...
%                    & \TPL{METIS_img_lmn_obs_AutoChopNod} \\
%                    & \TPL{METIS_img_lmn_obs_GenericChopNod} \\
  Input data:       & \NEWPROD{LM_SCI_BASIC_REDUCED}                               \\
                    & \NEWPROD{LM_STD_BASIC_REDUCED}                               \\
  Matched keywords: & dither position (\CODE{SKY}?                         \\
  Algorithm:        & Average all or \CODE{SKY} exposures with object rejection \\
                    & Subtract background                                       \\
  Output data:      & \NEWPROD{LM_SCI_BKG}                                         \\
                    & \NEWPROD{LM_STD_BKG}                                         \\
                    & \NEWPROD{LM_SCI_BKG_SUBTRACTED}                              \\
                    & \NEWPROD{LM_STD_BKG_SUBTRACTED}                              \\
                    & \NEWPROD{LM_SCI_OBJECT_CAT}                                  \\
                    & \NEWPROD{LM_STD_OBJECT_CAT}                                  \\
...
%   Type:                & Calibration                                                                  \\
%   Templates:           & \TPL{METIS_img_lm_cal_standard}                                              \\
%   Input data:          & \NEWPROD{LM_STD_BKG_SUBTRACTED}                                                 \\
...
%   Output data:         & \NEWPROD{LM_STD_AST_CALIB}                                                      \\
%                        & \NEWPROD{ASTROMETRY_TAB}                                                        \\
...
\begin{recipedef}\label{rec:metis_lm_img_std_process}
  Name:                & \NEWREC{metis_lm_img_std_process}                                               \\
...
%  Templates:           & \TPL{METIS_img_lm_cal_standard}                                              \\
  Input data:          & \NEWPROD{LM_STD_BKG_SUBTRACTED}                                                 \\
                       & \NEWEXTCALIB{FLUXSTD_CATALOG} (photometric standard catalogue) \\
  Matched keywords:    & OBJECT ID                                                                    \\
                       & FILTER ID                                                                    \\
  Parameters:          & None                                                                         \\
  Algorithm:           & Call \NEWDRL{lm_calculate_std_flux} to measure flux in input images                      \\
%                       & call \NEWDRL{recentre_img} to recentre and stack images                         \\
                       & call \CODE{hdrl_resample_compute} to recenter the images \\
                       & call \CODE{hdrl_imagelist_collapse} to stack the images \\
                       & call \NEWDRL{lm_calculate_std_flux} on the stacked image to get flux of the star in detector units\\
                       & call \NEWDRL{calculate_std_fluxcal} to calculate the conversion factor to physical units    \\
                       & call \NEWDRL{calculate_detection_limits} to compute measure background noise (std,rms) and compute detection limits \\
  Output data:         & \NEWPROD{LM_STD_COMBINED}                                                       \\
                       & \NEWPROD{FLUXCAL_TAB}                                                           \\
...
\begin{figure}[hb]
  \centering
   \includegraphics[width=0.9\textwidth]{metis_lm_img_std_process}
  %\resizebox{0.6\textwidth}{0.1\textwidth}{\TODO{\fbox{Figure to be done}}}
...
\begin{recipedef}
  Name:              & \NEWREC{metis_lm_img_calibrate}                     \\
  Purpose:           & Convert science images to physical units         \\
                     & Add distortion information                       \\
  Type:              & Calibration                                      \\
  Templates          & None                                             \\
  Input data:        & \NEWPROD{LM_SCI_BKG_SUBTRACTED}                     \\
                     & \NEWPROD{FLUXCAL_TAB}                               \\
                     & \NEWPROD{LM_DISTORTION_TABLE}                       \\
  Matched keywords:  & Filter ID                                        \\
  Parameters:        & None                                             \\
  Algorithm:         & call \NEWDRL{lm_scale_image_flux} to Scale image data to ph/s \\
                     & add header information (\FITS{BUNIT}, WCS, etc.) \\
  Output data:       & \NEWPROD{LM_SCI_CALIBRATED}                         \\
  QC1 parameters:    & None                                             \\
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=0.9\textwidth]{lm_img_calibrate}
...
\begin{recipedef}\label{rec:metis_lm_img_sci_postprocess}
  Name:                & \NEWREC{metis_lm_img_sci_postprocess}                         \\
  Purpose:             & Coadd reduced images.                                      \\
  Requirements:        & \REQ{METIS-6104}                                           \\
  Templates:           & None                                                       \\
  Type:                & Science                                                    \\
  Input data:          & \NEWPROD{LM_SCI_CALIBRATED} (Calibrated science images)       \\
                       & \NEWPROD{BADPIX_MAP_2RG} (Associated bad-pixel maps)           \\
  Parameters:          & None                                                       \\
  Algorithm:           & Check and refine WCS of input images by using the \ac{WFS-FS} data. \\
                       ...
                       & Call \CODE{hdrl_resample_compute} to recenter the images. \\
                       & Call \CODE{hdrl_imagelist_collapse} to stack the images. \\
  Output data:         & \NEWPROD{LM_SCI_COADD} (coadded, mosaiced image)              \\
% TheLM_SCI_COADD_ERROR and LM_SCI_COADD_CONTRIB can be layers in LM_SCI_COADD
%                        & \NEWPROD{LM_SCI_COADD_ERROR} (coadded, mosaiced error image)  \\
%                        & \NEWPROD{LM_SCI_COADD_CONTRIB} (contribution map)             \\
...
\begin{figure}[hb]
  ...
  \caption[Recipe: \REC{metis_lm_img_sci_postprocess}]{%
    \NEWREC{metis_lm_img_sci_postprocess} -- post-processing (coaddition)
...
\begin{recipedef}
  Name:                & \NEWREC{metis_lm_img_distortion}                                   \\
  ...
  Type:                & Calibration                                                     \\
  Input data:          & \NEWRAW{LM_DISTORTION_RAW} (Images of grid mask in WCU-FP2 or CFO-FP2.)\\
                       & \NEWRAW{LM_WCU_OFF_RAW} \\
                       & \NEWEXTCALIB{PINHOLE_TABLE} (Grid of pinhole mask positions) \\
                       & \NEWPROD{BADPIX_MAP_2RG} \\
...
%                       & \TBD \\
  ...
  Output data:         & \NEWPROD{LM_DISTORTION_TABLE} \\
                       & \NEWPROD{LM_DISTORTION_MAP}        \\
                       & \NEWPROD{LM_DIST_REDUCED}               \\
...
\begin{figure}[hb]
    ...
    \NEWREC{metis_lm_img_distortion} -- LM IMG distortion calibration}
