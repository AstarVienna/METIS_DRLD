...
$\mathrm{ADU\,s^{-1}}$ to $\mathrm{photons\,\,s^{-1}}$.
...
\begin{recipedef}
  ...
  Output data:         & \PROD{LM_STD_COMBINED}                                                       \\
                       & \PROD{FLUXCAL_TAB}                                                           \\
  Expected accuracies: & \TBD                                                                         \\
  QC1 parameters:      & \QC{QC LM IMG STD BACKGD RNS}                                                \\
...
values are in units of $\mathrm{photons\,\,s^{-1}}$. The header of
each file receives keyword \FITS{BUNIT} with value %
\CODE{'ph / s'}.

\TODO{Other units may be possible, although additional information is
  needed. For instance,\\ \CODE{'ph / (s arcsec2)'} makes values
  independent of the pixel scale, but requires a distortion map
...
\begin{figure}[hb]
  ...
  % \includegraphics[width=0.6\textwidth]{metis_lm_img_std_process}
  \resizebox{0.6\textwidth}{0.1\textwidth}{\TODO{\fbox{Figure to be done}}}
  \caption[Recipe: \REC{metis_lm_img_std_process}]{\REC{metis_lm_img_fluxcal} --
    convert images to physical flux units}
  \label{fig:metis_lm_img_fluxcal}
\end{figure}
...
\begin{recipedef}
  ...
  Input data:          & Calibrated science images (\PROD{LM_SCI_CALIBRATED})       \\
                       & Associated bad-pixel maps (\PROD{LM_SCI_BADPIX})           \\
  Parameters:          & None (TBD).                                                \\
...
\begin{recipedef}
  ...
  Output data:         & \PROD{LM_DISTORTION_TABLE} (table with polynomial coefficients) \\
                       & \PROD{LM_DISTORTION_MAP} (pixel scale across detector)          \\
                       & \PROD{LM_DIST_REDUCED} (reduced grid mask images)            \\
  Expected accuracies: & TBD                                                          \\
