...
The recipe \NEWREC{metis_LM_adc_slitloss} aims to determine the slit losses induced by the fixed \ac{ADC} positions as function of the object position across the slit. The recipe aims to create a table with slitlosses (\NEWSTATCALIB{LM_ADC_SLITLOSS}), which is added to the static database and used in the recipes \NEWREC{metis_LM_lss_std}. This recipe is to be carried out only when an update of the database is needed. The algorithm and the workflow of the recipe to determine the slit-losses is given in Section~\ref{sssec:adc_slitlosses}, more information can be found in Section "Calibration of slit-losses" in the Calibration Plan~\cite{METIS-calibration_plan}. 


%------------------------------------------------------------------------------------------------------------------
\subsubsection{\REC{metis_LM_lss_rsrf}:  Flatfielding}\label{rec:metis_lm_lss_rsrf}
The recipe \NEWREC{metis_LM_lss_rsrf} aims to create a spectroscopic master flatfield for determining the pixel-to-pixel sensitivity and to enable the order location algorithm (\NEWREC{metis_LM_lss_trace}).
\begin{figure}[ht]
  \centering
...
Input data:     & $N\times$ \NEWRAW{LM_LSS_RSRF_RAW} \\
...
Output data:	& \NEWPROD{MASTER_LM_LSS_RSRF} (\FITS{PRO.CATG=MASTER_LM_LSS_RSRF}): master flatfield/\ac{RSRF} \\
                & \NEWPROD{MEDIAN_LM_LSS_RSRF_IMG}: median map (\ac{QC})\\
                & \NEWPROD{MEAN_LM_LSS_RSRF_IMG}: mean map (\ac{QC})\\
...
The recipe \NEWREC{metis_LM_lss_trace} aims at detecting the order and a polynomial fitting of the order location (see~\cite{pis02} and~\cite{pis21} for details on the algorithms). The detection and polynomial fitting is based on flatfield frames taken through a pinhole mask, which leads to individual pinhole traces along the entire dispersion direction.
...
\begin{recipedef}
Name:		&  \NEWREC{metis_LM_lss_trace} \\
Purpose:	& Detection of order location \\
Type:		& Calibration\\
Requirements: & None \\
Templates:           & \TPL{METIS_spec_lm_cal_rsrfpinh}  \\
Input data:     & $N\times$ \NEWRAW{LM_LSS_RSRF_PINH_RAW} \\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_2RG}  \\
                & \NEWPROD{GAIN_MAP_2RG}  \\
                & \NEWPROD{BADPIX_MAP_2RG}  \\
                & \NEWPROD{MASTER_DARK_2RG}  \\
                & \NEWPROD{MASTER_LM_LSS_RSRF} \\
Parameters: 	& exposure time, spectral setup (L or M band)\\
Algorithm:      & Detection of the order edges\\
                & Polynomial fitting\\
Output data:	& \NEWPROD{LM_LSS_TRACE} (\FITS{PRO.CATG=LM_LSS_TRACE}): order table\\
...
Input data: 	& \NEWRAW{LM_LSS_WAVE_RAW}\\
                ...
                & \NEWPROD{MASTER_DARK_2RG}  \\
                & \NEWPROD{MASTER_LM_LSS_RSRF} \\
                & \NEWPROD{LM_LSS_TRACE} \\
                & \NEWSTATCALIB{LASER_TAB} \\
                % & \NEWSTATCALIB{REF_AIRG_CAT} \\
Parameters: 	& exposure time, spectral setup (L or M band)\\
...
Output data:	& \NEWPROD{LM_LSS_CURVE} (\FITS{PRO.CATG=LM_LSS_CURVE}): Curvature properties (\ac{QC}) \\
                & \NEWPROD{LM_LSS_DIST_SOL} (\FITS{PRO.CATG=LM_LSS_DIST_SOL}): Distortion solution\\
                & \NEWPROD{LM_LSS_WAVE_GUESS} (\FITS{PRO.CATG=LM_LSS_WAVE_GUESS}): Wavelength first guess\\
Expected accuracies: & 1/10th of a pixel after post-processing\\
               & (cf.~\cite{METIS-calibration_plan}, R-MET-106, METIS-167, METIS-1371)\\
...
Input data: 	& \NEWRAW{LM_LSS_STD_RAW}\\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_2RG}  \\
                & \NEWPROD{GAIN_MAP_2RG}  \\
                & \NEWPROD{BADPIX_MAP_2RG}  \\
                & \NEWPROD{MASTER_DARK_2RG}  \\
                & \NEWPROD{MASTER_LM_LSS_RSRF} \\
                & \NEWPROD{LM_LSS_DIST_SOL} \\
                & \NEWPROD{LM_LSS_WAVE_GUESS} \\
                & \NEWEXTCALIB{AO_PSF_MODEL} \\
                & \NEWEXTCALIB{ATM_LINE_CAT} \\
%                & \NEWSTATCALIB{TSS_MODEL_CAT}\\
%                & \NEWSTATCALIB{TSS_CONT_TAB}\\
                & \NEWSTATCALIB{LM_ADC_SLITLOSS}\\
                & \NEWSTATCALIB{LM_SYNTH_TRANS}\\
...
Output data:	& \NEWPROD{LM_LSS_STD_OBJ_MAP}: Pixel map of object pixels (\ac{QC})\\
            	& \NEWPROD{LM_LSS_STD_SKY_MAP}: Pixel map of sky pixels (\ac{QC})\\
              	& \NEWPROD{LM_LSS_STD_1D}: coadded, wavelength calibrated, collapsed 1D spectrum\\
              	& \NEWPROD{LM_LSS_STD_WAVE}: Wavelength solution derived from the \ac{STD} star (optional)\\
            	& \NEWPROD{STD_TRANSMISSION}: Transmission function derived by means of the \ac{STD} (\ac{QC})\\        
                & \NEWPROD{MASTER_LM_RESPONSE}: response function (optionally including the transmission) \\
...
%                & \TPL{METIS_spec_lm_cal_SlitAdc}\\
                ...
                & \TPL{METIS_spec_lmn_obs_AutoChopNodOnSlit}\\
Input data: 	& \NEWRAW{LM_LSS_SCI_RAW}\\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_2RG}  \\
                & \NEWPROD{GAIN_MAP_2RG}  \\
                & \NEWPROD{BADPIX_MAP_2RG}  \\
                ...
                & \NEWEXTCALIB{ATM_LINE_CAT} \\
                & \NEWSTATCALIB{LM_ADC_SLITLOSS}\\
            	& \NEWPROD{STD_TRANSMISSION} (optional)\\             
                %& \NEWEXTCALIB{AO_PSF_MODEL} \\
                %& \NEWSTATCALIB{LSF_KERNEL}\\
                & \NEWPROD{MASTER_LM_RESPONSE} \\
Parameters: 	& exposure time, spectral setup (L or M band), target information, nodding parameters\\
...
Output data:	& \NEWPROD{LM_LSS_SCI_OBJ_MAP}: Pixel map of object pixels (\ac{QC})\\
            	& \NEWPROD{LM_LSS_SCI_SKY_MAP}: Pixel map of sky pixels (\ac{QC})\\
            	& \NEWPROD{LM_LSS_SCI_2D}: coadded, wavelength calibrated 2D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{LM_LSS_2d_coadd_wavecal}) \\
                & \NEWPROD{LM_LSS_SCI_1D}: coadded, wavelength calibrated 1D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{LM_LSS_1d_coadd_wavecal}) \\
                & \NEWPROD{LM_LSS_SCI_FLUX_2D}: coadded, wavelength + flux calibrated 2D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{LM_LSS_2d_coadd_wavecal}) \\
              	& \NEWPROD{LM_LSS_SCI_FLUX_1D}: coadded, wavelength + flux 1D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{LM_LSS_1d_coadd_wavecal}) \\
Expected accuracies: & for wavelength: 1/10th of a pixel after post-processing\\
            & (cf.~\cite{METIS-calibration_plan}, R-MET-106, METIS-167, METIS-1371)\\
            & for flux: 10\% over an atmospheric band \\
...
The telluric correction will be done with the package \texttt{molecfit}\footnote{\url{https://www.eso.org/sci/software/pipelines/molecfit/molecfit-pipe-recipes.html}}. It is realised in three individual recipes, \NEWREC{metis_LM_lss_mf_model}, which calculates the best-fit model, \NEWREC{metis_LM_lss_mf_calctrans}, which creates a synthetic transmission curve, and \NEWREC{metis_LM_lss_mf_correct}, which performs the actual telluric correction by means of the synthetic transmission.
...
\begin{recipedef}
Name:		& \NEWREC{metis_LM_lss_mf_model} \\
...
Templates:           & None\\
Input data: 	& \NEWPROD{LM_LSS_SCI_FLUX_1D}\\
                & \NEWSTATCALIB{LSF_KERNEL} \\
                & \NEWEXTCALIB{ATM_PROFILE} \\
                & \NEWEXTCALIB{ATM_LINE_CAT} \\
...
Output data:	& \NEWPROD{MF_BEST_FIT_TAB}: Table with best-fit parameters\\
...
\begin{recipedef}
Name:		& \NEWREC{metis_LM_lss_mf_calctrans} \\
Purpose:	& Calculation of the synthetic transmission \\
Type:		& Post-calibration\\
Requirements: & METIS-4051, METIS-6091 \\
Templates:           & None\\
Input data: 	& \NEWPROD{MF_BEST_FIT_TAB}: Table with best-fit parameters\\
                & \NEWSTATCALIB{LSF_KERNEL} \\
                & \NEWEXTCALIB{ATM_PROFILE} \\
                & \NEWEXTCALIB{ATM_LINE_CAT} \\
Parameters: 	& \texttt{molecfit} parameters (c.f.  \cite{molecfit})\\
Algorithm:      & Calculate the entire transmission curve by means of the best-fit parameters\\
Output data:	& \NEWPROD{LM_LSS_SYNTH_TRANS}: synth. transmission\\
Expected accuracies: & n/a\\
QC1 parameters: & cf.~\cite{molecfit}\\
\end{recipedef}
