...
Output data:	& \PROD{MASTER_LM_LSS_RSRF} (\FITS{PRO.CATG=MASTER_LM_LSS_RSRF}): master flatfield/\ac{RSRF} \\
...
Algorithm:      & Detection of the order edges\\
                & Polynomial fitting\\
Output data:	& \PROD{LM_LSS_TRACE} (\FITS{PRO.CATG=LM_LSS_TRACE}): order table\\
...
Output data:	& \PROD{LM_LSS_CURVE} (\FITS{PRO.CATG=LM_LSS_CURVE}): Curvature properties (\ac{QC}) \\
                & \PROD{LM_LSS_DIST_SOL} (\FITS{PRO.CATG=LM_LSS_DIST_SOL}): Distortion solution\\
                & \PROD{LM_LSS_WAVE_GUESS} (\FITS{PRO.CATG=LM_LSS_WAVE_GUESS}): Wavelength first guess\\
Expected accuracies: & 1/10th of a pixel after post-processing\\
               & (cf.~\cite{METIS-calibration_plan}, R-MET-106, METIS-167, METIS-1371)\\
...
The telluric correction will be done with the package \texttt{molecfit}\footnote{\url{https://www.eso.org/sci/software/pipelines/molecfit/molecfit-pipe-recipes.html}}. It is realised in three individual recipes, \REC{metis_LM_lss_mf_model}, which calculates the best-fit model, \REC{metis_LM_lss_mf_calctrans}, which creates a synthetic transmission curve, and \REC{metis_LM_lss_mf_correct}, which performs the actual telluric correction by means of the synthetic transmission.
