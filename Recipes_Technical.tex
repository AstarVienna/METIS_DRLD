...
Input data:     & \hyperref[dataitem:lm_image_raw]{\RAW{LM_IMAGE_RAW}} \\
...
\TODO{TODO: The following is a just suggestion, feel free to remove/edit. /TM"}
...
method to use will be chosen based on AIT tests (cf.~rationale for
\REQ{METIS-9151}).
...
Output data:	& Slit loss model as function of the wavelength and object position across the slit (\hyperref[dataitem:lm_adc_slitloss]{\STATCALIB{LM_ADC_SLITLOSS}}) \\
...
Algorithm:      & remove detector signature\\
                ...
                & apply aperture photometry\\
                & calculate (simple) slitloss model (details to be defined)\\
Output data:	& Slit loss model as function of the wavelength and object position across the slit (\hyperref[dataitem:n_adc_slitloss]{\STATCALIB{N_ADC_SLITLOSS}}) \\
Expected accuracies: & 3\% (cf. \cite{METIS_calerrbudget})\\
QC1 parameters: & \QC{TBD}: TBD\\
\end{recipedef}
