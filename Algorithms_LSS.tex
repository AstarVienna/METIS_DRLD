...
\end{figure}
\end{landscape}
For the flux loss compensation one needs to determine the slitloss functions (shown in Fig.~\ref{fig:slitlosses}). This can be done in the same way as described above As there is no \ac{PSF} reconstruction foreseen, we assume the following approaches to compensate the slitlosses:
\begin{itemize}
