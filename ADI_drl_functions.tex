...
%Output FITS files: & None \\
Outputs: & \texttt{hdrl\_image *psf\_median} (\PROD{lm_cgrph_PSF_MEDIAN})\\
         & \texttt{cpl\_error\_code} \\
...
QC outputs: & \QC{QC det cgrph SCI FWHM nn}\\
%Output FITS files: & None \\
Outputs: & \texttt{hdrl\_image *psf\_median} (\PROD{n_cgrph_PSF_MEDIAN})\\
         & \texttt{cpl\_error\_code} \\
General description: & Calculation of median PSF for sequence of ADI images\ \\
Mathematical description: & see Section~\ref{ssec:algo_app_imaging} \\
Quality assessment: & Through QC parameters \\
...
%Output FITS files: & None \\
Outputs: & \texttt{hdrl\_image *psf\_median} (\PROD{lm_app_PSF_MEDIAN})\\
         & \texttt{cpl\_error\_code} \\
General description: & Calculation of median PSF for sequence of ADI images\ \\
Mathematical description: & see Section~\ref{ssec:algo_app_imaging} \\
Quality assessment: & Through QC parameters \\
...
QC outputs: & None\\
%Output FITS files: & None \\
Outputs: & \texttt{hdrl\_image *output} (merged PSF) \\
         & \texttt{cpl\_error\_code} \\
...
Input: &\texttt{hdrl\_imagelist *input} \\
       ...
       & \texttt{cpl\_table *disttab} (\PROD{det_DISTORTION_TAB})\\
%Other inputs: & Parameters for regridding \\
QC outputs: & None\\
Outputs: & \texttt{hdrl\_image *output} (\PROD{det_cgrph_SCI_CENTRED}) \\
         & \texttt{cpl\_error\_code} \\
...
Outputs: & \texttt{hdrl\_image *output} (\PROD{det_cgrph_SCI_DEROTATED_PSFSUB}) \\
         & \texttt{hdrl\_image *output}  \PROD{det_cgrph_SCI_DEROTATED} \\
         & \texttt{hdrl\_image *psf\_median} (\PROD{det_cgrph_PSF_MEDIAN})\\
         & \texttt{cpl\_error\_code} \\
...
Function Parameters: None \\
Input: &\texttt{hdrl\_imagelist *input} \\
       & \texttt{hdrl\_image *input} (Off-axis PSF reference) \\
...
Function Parameters: None \\
Input: &\texttt{hdrl\_imagelist *input} \\
       & \texttt{hdrl\_image *psf} (Off-axis PSF reference) \\
