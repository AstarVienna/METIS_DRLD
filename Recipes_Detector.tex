...
The recipe \NEWREC{metis_det_lingain} determines detector (non-)linearity and absolute detector
...
\begin{recipedef}
  ...
  Input data:          & \NEWRAW{DETLIN_det_RAW}: (set of \FITS{FLAT,LAMP} frames taken with increasing DIT) \\
                       & \NEWRAW{det_WCU_OFF_RAW}: (set of internal darks taken at the start of the template) \\
%  Matched keywords:    & Subsystem ID \TODO{TBD}                                                             \\
  Algorithm:           & Subtract instrument dark (\CODE{hdrl_imagelist_sub_image}).                         \\
                       ...
                       & Gain is determined as the slope of variance against mean (\NEWDRL{metis_derive_gain}) \\
  ...
  Output data:         & \NEWPROD{GAIN_MAP_det}                                    \\
                       & \NEWPROD{LINEARITY_det}                                 \\
                       & \NEWPROD{BADPIX_MAP_det}                                \\
...
\begin{recipedef}
  ...
  Input data:          & \NEWRAW{DARK_det_RAW}  \\
                       & \NEWSTATCALIB{LINEARITY_det}  \\
% TODO: Remove persistence, because if the raw darks have persistence, we are doing something wrong
  ...
  Algorithm:           & Group files by detector and \texttt{DIT}, based on header keywords           \\
                       & Call function \NEWDRL{metis_determine_dark} for each set of files\\
                       & call \NEWDRL{metis_update_dark_mask} to flag deviant pixels \\
  Output data:         & \NEWPROD{MASTER_DARK_det}                                                      \\
...
%\begin{recipedef}
%  Name:                & \NEWREC{metis_det_persistence}           \\
...
%  Output data:         & \NEWPROD{PERSISTENCE_MAP}                \\
