...
\begin{recipedef}
  ...
  Input data:          & \RAW{DETLIN_RAW_det}: (set of \FITS{FLAT,LAMP} frames taken with increasing DIT)    \\
                       & \PROD{MASTER_DARK_det}: (set of instrument darks \TODO{use METIS_gen_cal_InsDark})  \\
  Matched keywords:    & Detector ID \TODO{TBD}                                                              \\
  Algorithm:           & Subtract instrument dark (\CODE{hdrl_imagelist_sub_image}).                         \\
                       & Compute mean and variance for each frame (\CODE{TBD}).                              \\
                       & Gain is determined as the slope of variance against mean (\CODE{metis_derive_gain}) \\
