...
Input: &  \texttt{hdrl\_image *input} \\
       &  \texttt{const hdrl\_image *dark} (\PROD{MASTER_DARK_det})\\
       &  \texttt{const hdrl\_image *flat}  (\PROD{MASTER_IMG_FLAT_LAMP_LM}, \PROD{MASTER_IMG_FLAT_LAMP_N}, \PROD{MASTER_IMG_FLAT_TWILIGHT_LM}, \PROD{MASTER_IMG_FLAT_TWILIGHT_N})\\
%Other inputs: & None \\
%QC outputs: & None\\
...
Outputs: & \texttt{const hdrl\_image *sky} (\PROD{LM_SCI_BKG}, \PROD{LM_STD_BKG}) \\
                & \texttt{cpl\_error\_code} \\
...
%Function Parameters: & None \\
Input: & \texttt{hdrl\_image *input} \\
       & \texttt{const hdrl\_image *sky} (\PROD{LM_SCI_BKG}, \PROD{LM_STD_BKG})\\
     %\texttt{const ls\_master_dark_image *input} \\
    %\texttt{const ls\_master_flat_image *input} \\
%Other inputs: & None \\
%QC outputs: & None\\
...
%Output FITS files: & None \\
Outputs: &  \texttt{hdrl\_image *flat} (\PROD{MASTER_IMG_FLAT_LAMP_LM}, \PROD{MASTER_IMG_FLAT_TWILIGHT_LM}) \\
         & \texttt{cpl\_error\_code} \\
...
%Output FITS files: & None \\
Outputs: &  \texttt{hdrl\_image *flat} (\PROD{MASTER_IMG_FLAT_LAMP_N}, \PROD{MASTER_IMG_FLAT_TWILIGHT_N}) \\
         & \texttt{cpl\_error\_code} \\
General description: & Determination of master flat for flat fielding \\
Mathematical description: & Combine images with the same DIT, fit slope of pixels against illumination level \\
Quality assessment: & Through QC parameters \\
...
%QC outputs: & None\\
%Output FITS files: & None \\
Outputs: &  \texttt{hdrl\_image *linearity} (\PROD{LINEARITY_det}) \\
               & \texttt{cpl\_error\_code} \\
...
Outputs:    & \texttt{cpl\_table *dist} (\PROD{det_DIST_REDUCED})\\
            &  \texttt{hdrl\_image *distmap}  (\PROD{det_DISTORTION_MAP})\\
