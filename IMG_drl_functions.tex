...
\begin{recipedef}
Name: & \NEWDRL{img_detrend} \\
Purpose: &Remove detector signals\\
Used in recipes: & \NEWREC{metis_lm_img_basic_reduce}\\
...
\begin{recipedef}
Name: & \NEWDRL{img_skybackground_build} \\
Purpose: &Building thermal background signals\\
Used in recipes: & \NEWREC{metis_lm_img_background}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & \texttt{const hdrl\_image * input} \\
...
\begin{recipedef}
Name: & \NEWDRL{img_skybackground_removal} \\
...
%\begin{recipedef}
%Name: & \NEWDRL{img_source_extraction} \\
...
%\begin{recipedef}
%Name: & \NEWDRL{img_resample_coadding} \\
...
\begin{recipedef}
Name: & \NEWDRL{metis_lm_img_flat} \\
Purpose: & Create master flat field for the LM-band imaging detector\\
Used in recipes: & \NEWREC{metis_lm_img_flat}\\
%Working remarks: & None \\
%Function Parameters: & None \\
...
\subsubsection{Create N Master Flat}\label{drl:n_img_flat}\label{drl:metis_n_img_flat}
\begin{recipedef}
Name: & \NEWDRL{metis_n_img_flat} \\
Purpose: & Create master flat field for the N-band imaging detector\\
Used in recipes: & \NEWREC{metis_n_img_flat}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & $n\times$ \texttt{const hdrl\_image * input} \\
Other inputs: &  combination method (\texttt{median}, \texttt{mean}, \texttt{sigclip},\dots)\\
...
\begin{recipedef}
Name: & \NEWDRL{metis_update_lm_flat_mask} \\
...
\begin{recipedef}
Name: & \NEWDRL{metis_update_n_flat_mask} \\
...
\begin{recipedef}
Name: & \NEWDRL{metis_derive_gain} \\
Purpose: & Determine the gain. \\
Used in recipes: & \NEWREC{metis_det_lingain}\\
%Working remarks: & None \\
%Function Parameters: & None \\
...
\begin{recipedef}
Name: & \NEWDRL{metis_derive_nonlinearity} \\
...
\begin{recipedef}
Name: & \NEWDRL{lm_calculate_std_flux} \\
Purpose: & Calculate LM band flux and position of standard star in detector units \\
Used in recipes: & \NEWREC{metis_lm_img_std_process}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & $n\times$ \texttt{const hdrl\_image * input} \\
...
%Output FITS files: & \NEWPROD{LM_STD_COMBINED} \\
...
\begin{recipedef}
Name: & \NEWDRL{n_calculate_std_flux} \\
Purpose: & Calculate N band flux and position of standard star in detector units \\
Used in recipes: & \NEWREC{metis_n_img_std_process}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & \texttt{const hdrl\_image * input} \\
%Other inputs: & photometric standard catalogue \\
...
\begin{recipedef}
Name: & \NEWDRL{calculate_std_fluxcal} \\
...
\begin{recipedef}
Name: & \NEWDRL{calculate_detection_limits} \\
Purpose: & Calculate Detection Limits \\
Used in recipes: & \NEWREC{metis_lm_img_std_process}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: &  \texttt{const hdrl\_image * input} \\
Other inputs: & \NEWPROD{FLUXCAL_TAB} \\
...
\begin{recipedef}
Name: & \NEWDRL{detect_centroid_peak} \\
Purpose: &Detect the location of a source peak by a centroid\\
Used in recipes: & \NEWREC{metis_img_chophome}\newline
...
\begin{recipedef}
Name: & \NEWDRL{lm_scale_image_flux} \\
Purpose: & Calculate the conversion between instrumental and physical flux units \\
Used in recipes: & \NEWREC{metis_lm_img_calibrate}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & \NEWPROD{LM_SCI_BKG_SUBTRACTED}\\
         \NEWPROD{FLUXCAL_TAB} \\
Other inputs: & None \\
QC outputs: & None\\
%Output FITS files: & \NEWPROD{LM_STD_COMBINED} \\
...
\begin{recipedef}
Name: & \NEWDRL{lm_update_header_distortion} \\
...
QC outputs: & None \\
Output FITS files: & \NEWPROD{LM_SCI_CALIBRATED} \\
Outputs: & \texttt{cpl\_error\_code} \\
...
\begin{recipedef}
Name: & \NEWDRL{n_scale_image_flux} \\
Purpose: & Calculate the conversion between instrumental and physical flux units \\
Used in recipes: & \NEWREC{metis_n_img_calibrate}\\
%Working remarks: & None \\
%Function Parameters: & None \\
Input: & \NEWPROD{N_SCI_BKG_SUBTRACTED}\\
         \NEWPROD{FLUXCAL_TAB} \\
Other inputs: & None \\
QC outputs: & None\\
%Output FITS files: & \NEWPROD{N_STD_COMBINED} \\
Outputs: & \texttt{cpl\_error\_code} \\
General description: & Convert instrumental flux to ph/s \\
...
\begin{recipedef}
Name: & \NEWDRL{n_update_header_distortion} \\
Purpose: & Update the FITS header with calibration data (WCS, distortion, units)  \\
Used in recipes: & \NEWREC{metis_n_img_calibrate}\\
%Working remarks: & None \\
%Function Parameters: & None \\
...
QC outputs: & None \\
Output FITS files: & \NEWPROD{N_SCI_CALIBRATED} \\
Outputs: & \texttt{cpl\_error\_code} \\
...
Input: &   Catalogue of detected sources \\
       &   \NEWEXTCALIB{PINHOLE_TABLE}\\
QC outputs: & \QC{QC det DISTORT RMS}  \\
            & \QC{QC det DISTORT NSOURCE}  \\
Output FITS files: & \NEWPROD{det_DIST_REDUCED}\\
                   & \NEWPROD{det_DISTORTION_MAP}\\
Outputs:  &  \NEWPROD{det_DISTORTION_TABLE} \\
          & \texttt{cpl\_error\_code} \\
...
\begin{recipedef}
Name: & \NEWDRL{cutout_region} \\
Purpose: & Cutout a region around a beam in N band image  \\
Used in recipes: & \NEWREC{metis_n_img_restore}\\
%Working remarks: & None \\
Function Parameters: & Size of cutout region \\
Input: &   \NEWPROD{N_SCI_CALIBRATED} \\
QC outputs: None \\
%Output FITS files:\NEWPROD{N_SCI_RESTORED} \\
%Outputs:  &  \NEWPROD{det_DISTORTION_TABLE} \\
%          & \texttt{cpl\_error\_code} \\
