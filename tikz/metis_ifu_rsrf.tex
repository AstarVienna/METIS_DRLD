...
\begin{tikzpicture}
  ...
  \node (step_detrend) [below=5cm of input, redstep]{%
    detector signature\\
    removal
  };
  ...
  %% Input
  \node (connect_bpm) [connection] at ($(input)!0.2!(step_detrend)$) {};
  \node (bpm) [left=of connect_bpm, external] {\EXTCALIB{BADPIX_MAP_IFU}};
  \draw [connection_arrow, dashed] (bpm) -- (connect_bpm);

  \node (connect_persistence) [connection] at ($(input)!0.4!(step_detrend)$) {};
