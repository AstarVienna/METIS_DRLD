...
\begin{tikzpicture}[on grid=false, node distance=0.8cm]
  ...
  \matrix (recipes) [column sep=1mm, row sep=1cm]{
    ...
    \node[above] (REClin_raw){\recipebox{\NEWRAW{IFU_LINEARITY}}{\NEWREC{metis_ifu_linearity}}}; &
    \node[above] (RECpers_raw){\recipebox{\NEWRAW{PERSISTENCE}}{\NEWREC{persistence_map}}}; &
...
%    \node (adi_adi)[scienceproduct]{ADI\_SCI\_COADD};
  ...
  \draw [match] (REClin_DIlin) -- (RECscireduce_DIlin);
