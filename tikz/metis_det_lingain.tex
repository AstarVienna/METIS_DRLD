...
%   \draw[help lines, green](-5, 0) grid (8, 11);
  ...
  \node (input) [below=0.75cm of start-m, input] {%
    \textsl{N} \hyperref[dataitem:detlin_det_raw]{\RAW{DETLIN_det_RAW}}\\
    \hyperref[dataitem:det_wcu_off_raw]{\RAW{det_WCU_OFF_RAW}}};
  ...
  \node (gainmap) [right=of connectgain, calproduct]{%
    \hyperref[dataitem:gain_map_det]{\STATCALIB{GAIN_MAP_det}}};
  ...
  \node (bpm) [right=of connectbpm, calproduct]{%
    \hyperref[dataitem:badpix_map_det]{\STATCALIB{BADPIX_MAP_det}}};
  \draw (connectbpm) -- (bpm);
