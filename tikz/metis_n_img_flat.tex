...
%   \draw[help lines, green](-5, 0) grid (8, 11);
  ...
  \node (input) [below=0.75cm of start-m, input] {%
    \textsl{N} \hyperref[dataitem:n_flat_lamp_raw]{\RAW{N_FLAT_LAMP_RAW}}\\
    or \textsl{N} \hyperref[dataitem:n_flat_twilight_raw]{\RAW{N_FLAT_TWILIGHT_RAW}}};
...
%   \draw (params) -- (connectparams);
  ...
  \node (bpmin) [left=of connectbpm, calproduct]{%
    \hyperref[dataitem:badpix_map_geo]{\STATCALIB{BADPIX_MAP_GEO}}};
  ...
  \node (darkin) [left=of connection_masterdark, calproduct]{%
    \hyperref[dataitem:master_dark_geo]{\STATCALIB{MASTER_DARK_GEO}}};
  \draw (darkin) -- (connection_masterdark);

  \node (connectflat) [connection] at
  ($(step_fit)!0.3!(stop-t)$){};
  \node (flatout) [right=of connectflat,calproduct]{%
    \hyperref[dataitem:master_img_flat_lamp_n]{\STATCALIB{MASTER_IMG_FLAT_LAMP_N}}\\
    or \hyperref[dataitem:master_img_flat_twilight_n]{\STATCALIB{MASTER_IMG_FLAT_TWILIGHT_N}}};
