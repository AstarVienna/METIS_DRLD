...
%   \draw[help lines, green](-5, 0) grid (8, 11);
  ...
  \node (input) [below=0.75cm of start-m, input] {\textsl{N} \hyperref[dataitem:dark_det_raw]{\RAW{DARK_det_RAW}}};
  \node (step_linearity) [below=2.0cm of input, redstep] {%
    apply non-linearity correction};

  \node (connectlinearity) [connection] at ($(input)!0.65!(step_linearity)$){};
  \node (linearity) [left=of connectlinearity, external]{\EXTCALIB{LINEARITY_det}};
  ...
  \node (masterdark) [right=of connectdark, calproduct] {\hyperref[dataitem:master_dark_det]{\STATCALIB{MASTER_DARK_det}}};
  \draw (connectdark) -- (masterdark);

  \node (connecthot) [connection] at ($(step_thresholding)!0.5!(stop-t)$){};
  \node (hot) [right=of connecthot, calproduct] {\hyperref[dataitem:badpix_map_det]{\STATCALIB{BADPIX_MAP_det}}};
  \draw (connecthot) -- (hot);
