...
\begin{recipedef}
  ...
  Input data:          & \hyperref[dataitem:n_flat_lamp_raw]{\RAW{N_FLAT_LAMP_RAW}} or \hyperref[dataitem:n_flat_twilight_raw]{\RAW{N_FLAT_TWILIGHT_RAW}} \\
                       & \hyperref[dataitem:master_dark_geo]{\PROD{MASTER_DARK_GEO}} (for twilight flats)                               \\
...
%                       & possibly others (e.g.\ coronagraphic mask, \TBD)               \\
  ...
  Algorithm:           & Call \hyperref[drl:metis_apply_persistence_correction]{\DRL{metis_apply_persistence_correction}} to apply the
                       ...
                       & Call \hyperref[drl:metis_update_n_flat_mask]{\DRL{metis_update_n_flat_mask}} to flag deviant pixels. \\
  Output data:         & \hyperref[dataitem:master_img_flat_lamp_n]{\PROD{MASTER_IMG_FLAT_LAMP_N}} or \hyperref[dataitem:master_img_flat_twilight_n]{\PROD{MASTER_IMG_FLAT_TWILIGHT_N}} \\
                       & \hyperref[dataitem:badpix_map_geo]{\PROD{BADPIX_MAP_GEO}}                                            \\
...
%                     & \TPL{METIS_img_n_clc_obs_FixedSkyOffset}                     \\
  ...
  Input data:        & \hyperref[dataitem:n_image_sci_raw]{\RAW{N_IMAGE_SCI_RAW}} (Chopped/nodded science or standard images)\\
                     ...
                     & or \hyperref[dataitem:master_img_flat_twilight_n]{\PROD{MASTER_IMG_FLAT_TWILIGHT_N}} \\
...
% TODO: Better no figure than the wrong figure.
% \begin{figure}[hb]
%   \centering
...
%  Templates:           & \TPL{METIS_img_n_cal_standard}                                                \\
  Input data:          & \hyperref[dataitem:n_std_bkg_subtracted]{\PROD{N_STD_BKG_SUBTRACTED}}                                                   \\
                       & \hyperref[dataitem:fluxstd_catalog]{\EXTCALIB{FLUXSTD_CATALOG}} \\
  Matched keywords:    & Object ID                                                                     \\
                       & Filter ID                                                                     \\
  Parameters:          & None                                                                          \\
  Algorithm:           & call \hyperref[drl:n_calculate_std_flux]{\DRL{n_calculate_std_flux}} to measure the flux in all beams\\
                       & average and normalize flux values \\
                       & call \hyperref[drl:calculate_std_fluxcal]{\DRL{calculate_std_fluxcal}} to calculate conversion factor to physical units   \\
                       & call \hyperref[drl:calculate_detection_limits]{\DRL{calculate_detection_limits}} to compute measured background noise (std, rms) and compute detection limits \\
...
\begin{figure}[hb]
  \centering
   \includegraphics[width=0.9\textwidth]{metis_n_img_std_process}
  %\resizebox{0.6\textwidth}{0.1\textwidth}{\TODO{\fbox{Figure to be done}}}
...
\begin{recipedef}
  ...
  Matched keywords:  & Filter ID                                        \\
  Parameters:        & None                                             \\
  Algorithm:         & call \hyperref[drl:n_scale_image_flux]{\DRL{n_scale_image_flux}} to Scale image data to ph/s \\
...
\begin{figure}[hb]
  \centering
   \includegraphics[width=0.9\textwidth]{metis_n_img_calibrate}
...
\begin{recipedef}\label{rec:metis_n_img_restore}
  ...
  Algorithm:         & Call \hyperref[drl:cutout_region]{\DRL{cutout_region}} to cut regions around beams                \\
...
\begin{recipedef}
  ...
  Type:                & Calibration                                                    \\
  Input data:          & \hyperref[dataitem:n_distortion_raw]{\RAW{N_DISTORTION_RAW}} (Images of grid mask in WCU-FP2 or CFO-FP2.)\\
  ...
  QC1 parameters:      & \hyperref[qc:qc_n_distort_rms]{\QC{QC N DISTORT RMS} }                                         \\
                       & \hyperref[qc:qc_n_distort_nsource]{\QC{QC N DISTORT NSOURCE}}  \\
  hdrl functions:      & \CODE{hdrl_catalogue_create}                                   \\
                       & \CODE{hdrl_imagelist_sub_image}                                \\
\end{recipedef}
