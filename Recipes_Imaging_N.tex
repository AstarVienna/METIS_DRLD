...
\begin{recipedef}
  ...
  Input data:          & \NEWRAW{N_FLAT_LAMP_RAW} or \NEWRAW{N_FLAT_TWILIGHT_RAW} \\
                       & \NEWPROD{MASTER_DARK_GEO} (for twilight flats)                               \\
...
%                       & possibly others (e.g.\ coronagraphic mask, \TBD)               \\
  ...
  Algorithm:           & Call \NEWDRL{metis_apply_persistence_correction} to apply the
                         persistence correction \\
                       & For internal flats: call \NEWREC{metis_det_dark} with \CODE{LAMP OFF}
                       images to create dark frame. \\
                       & Subtract internal dark or master dark from flat exposures.     \\
                       & call \NEWREC{metis_n_img_flat} to fit slope of pixel values against
                       ...
                       & Call \NEWDRL{metis_update_n_flat_mask} to flag deviant pixels. \\
  Output data:         & \NEWPROD{MASTER_IMG_FLAT_LAMP_N} or \NEWPROD{MASTER_IMG_FLAT_TWILIGHT_N} \\
                       & \NEWPROD{BADPIX_MAP_GEO}                                            \\
...
\begin{recipedef}
  Name:              & \NEWREC{metis_n_img_chopnod}                                    \\
...
%                     & \TPL{METIS_img_n_clc_obs_FixedSkyOffset}                     \\
  ...
  Input data:        & \NEWRAW{N_IMAGE_SCI_RAW} (Chopped/nodded science or standard images)\\
                     & \NEWPROD{BADPIX_MAP_GEO} Bad-pixel map \\
                     & \NEWPROD{MASTER_IMG_FLAT_LAMP_N} \\
                     & or \NEWPROD{MASTER_IMG_FLAT_TWILIGHT_N} \\
  ...
  Output data:       & \NEWPROD{N_SCI_BKG_SUBTRACTED}                                  \\
                     & \NEWPROD{N_STD_BKG_SUBTRACTED}                                  \\
...
\begin{recipedef}
  Name:                & \NEWREC{metis_n_img_std_process}                                                 \\
...
%  Templates:           & \TPL{METIS_img_n_cal_standard}                                                \\
  Input data:          & \NEWPROD{N_STD_BKG_SUBTRACTED}                                                   \\
                       & \NEWEXTCALIB{FLUXSTD_CATALOG} \\
  Matched keywords:    & Object ID                                                                     \\
                       & Filter ID                                                                     \\
  Parameters:          & None                                                                          \\
  Algorithm:           & call \NEWDRL{n_calculate_std_flux} to measure the flux in all beams\\
                       & average and normalize flux values \\
                       & call \NEWDRL{calculate_std_fluxcal} to calculate conversion factor to physical units   \\
                       & call \NEWDRL{calculate_detection_limits} to compute measured background noise (std, rms) and compute detection limits \\
  Output data:         & \NEWPROD{FLUXCAL_TAB}                                                            \\
...
\begin{recipedef}
  Name:              & \NEWREC{metis_n_img_calibrate}                      \\
  Purpose:           & Convert science image to physical units          \\
                     & Add distortion information                       \\
  Type:              & Calibration                                      \\
  Templates:         &                                                  \\
  Input data:        & \NEWPROD{N_SCI_BKG_SUBTRACTED}                      \\
                     & \NEWPROD{FLUXCAL_TAB}                               \\
                     & \NEWPROD{N_DISTORTION_TABLE}                        \\
  Matched keywords:  & Filter ID                                        \\
  Parameters:        & None                                             \\
  Algorithm:         & call \NEWDRL{n_scale_image_flux} to Scale image data to ph/s \\
                     & add header information (\FITS{BUNIT}, etc.)\\
  Output data:       & \NEWPROD{N_SCI_CALIBRATED}                          \\
  QC1 parameters:    & None                                             \\
\end{recipedef}
...
\begin{recipedef}
  Name:                & \NEWREC{metis_n_img_distortion}                                   \\
  Purpose:             & Determine optical distortion coefficients for the N imager.    \\
  Templates:           & \TPL{METIS_img_n_cal_distortion}                               \\
  Type:                & Calibration                                                    \\
  Input data:          & \NEWRAW{N_DISTORTION_RAW} (Images of grid mask in WCU-FP2 or CFO-FP2.)\\
  ...
  Algorithm:           & Subtract background image.  (\CODE{hdrl_imagelist_sub_image})                                       \\
                       ...
                       & call \NEWDRL{fit_distortion} to fit polynomial coefficients to deviations from grid positions. \\
  Output data:         & \NEWPROD{N_DISTORTION_TABLE} \\
                       & \NEWPROD{N_DISTORTION_MAP}        \\
                       & \NEWPROD{N_DIST_REDUCED}             \\
...
\begin{figure}[hb]
  ...
  \caption[Recipe: \REC{metis_n_img_distortion}]{%
    \NEWREC{metis_n_img_distortion} -- \CODE{IMG_N} distortion calibration}
