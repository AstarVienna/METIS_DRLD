...
The scientific and technical performance requirements for METIS, as outlined in the Top Level Requirements (TLRs), establish the standards for data processing by the METIS pipeline. These standards ensure that the pipeline processes data to the precise levels necessary for accurate and reliable scientific output. The pipeline must not only meet these high-performance benchmarks but also seamlessly integrate with various activities associated with Assembly, Integration, and Verification (AIV).
...
ESO is currently implementing new pipeline toolchains, specifically EDPS and PyCPL, both of which are based on Python. This transition affects the METIS pipeline development, as the implementation efforts must align with these new standards. Utilizing EDPS and PyCPL will likely streamline the development process, enhance integration with existing ESO tools, and ensure future compatibility and support.
