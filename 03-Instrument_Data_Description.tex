...
\begin{itemize}
\item When the number of (primary) output FITS files is the same as the number of (primary) input FITS files, then relevant FITS header keywords are copied per FITS files. When there is only one output FITS file for a set of input FITS files (e.g. when stacking frames for a \hyperref[dataitem:master_dark_2rg]{\PROD{MASTER_DARK_2RG}}), then only FITS headers from the first input file are propagated.
