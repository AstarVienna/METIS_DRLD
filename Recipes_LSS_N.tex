...
Input data: 	& \RAW{N_LSS_SCI_RAW}\\
                ...
                & \PROD{STD_TRANSMISSION}: Transmission curve derived by means of the \ac{STD} star Matched Keywords \\
                & \FITS{DET.DIT} (optionally) \\
                & \FITS{DET.NDIT}\\
                & \FITS{DRS.SLIT}\\
Parameters: 	& exposure time, target information, nodding(/chopping parameters\\
...
Output data:	& \PROD{N_LSS_SCI_OBJ_MAP}: Pixel map of object pixels (\ac{QC})\\
                ...
                & (\FITS{PRO.CATG}: \CODE{N_LSS_1d_coadd_wavecal}) \\
              	& \PROD{N_LSS_SCI_FLUX_TELL_1D}: coadded, wavelength, telluric corrected 1D spectrum\\
