...
The recipe \NEWREC{metis_n_adc_slitloss} aims to determine the slit losses induced by atmospheric refraction. The recipe aims to create a table with slitlosses (\NEWSTATCALIB{N_ADC_SLITLOSS}), which is added to the static database and used in the recipes \NEWREC{metis_N_lss_std}. This recipe is to be carried out only when an update of the database is needed. The algorithm and the workflow of the recipe to determine the slitlosses is given in Section~\ref{sssec:adc_slitlosses}, more information can be found in Section "Calibration of slit losses" in the Calibration Plan~\cite{METIS-calibration_plan}. 

%------------------------------------------------------------------------------------------------------------------
\subsubsection{\REC{metis_N_lss_rsrf}:  Flatfielding}\label{rec:metis_n_lss_rsrf}
The recipe \NEWREC{metis_N_lss_rsrf} aims to create a spectroscopic master flatfield for determining the pixel-to-pixel sensitivity and to enable the order location algorithm (\NEWREC{metis_N_lss_trace}).
\begin{figure}[ht]
  \centering
...
Input data:     & $N\times$ \NEWRAW{N_LSS_RSRF_RAW} \\
...
Output data:	&  \NEWPROD{MASTER_N_LSS_RSRF} (\FITS{PRO.CATG=MASTER_N_LSS_TRSRF}): master flatfield/\ac{RSRF} \\
                & \NEWPROD{MEDIAN_N_LSS_RSRF_IMG}: median map (\ac{QC})\\
                & \NEWPROD{MEAN_N_LSS_RSRF_IMG}: mean map (\ac{QC})\\
...
The recipe \NEWREC{metis_N_lss_trace} aims at detecting the order and a polynomial fitting of the order location (see~\cite{pis02} and~\cite{pis21} for details on the algorithms). The detection and polynomial fitting is based on flatfield frames taken through a pinhole mask, which leads to individual pinhole traces along the entire dispersion direction.
...
\begin{recipedef}
Name:		& \NEWREC{metis_N_lss_trace} \\
Purpose:	& Detection of order location \\
Type:		& Calibration\\
Requirements: & None \\
Templates:           & \TPL{METIS_spec_n_cal_rsrfpinh} \\
Input data:     & $N\times$ \NEWRAW{N_LSS_RSRF_PINH_RAW} \\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_GEO}  \\
                & \NEWPROD{GAIN_MAP_GEO}  \\
                & \NEWPROD{BADPIX_MAP_GEO}   \\
                & \NEWPROD{MASTER_DARK_GEO}  \\
                &  \NEWPROD{MASTER_N_LSS_RSRF} \\
Parameters: 	& exposure time\\
Algorithm:      & Detection of the order edges\\
                & Polynomial fitting\\
Output data:	& \NEWPROD{N_LSS_TRACE} (\FITS{PRO.CATG=N_LSS_TRACE}): Polynomial coefficients\\
...
Input data: 	& \NEWRAW{N_LSS_STD_RAW}\\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_GEO}  \\
                & \NEWPROD{GAIN_MAP_GEO}  \\
                & \NEWPROD{BADPIX_MAP_GEO}   \\
...
Output data:	& \NEWPROD{N_LSS_STD_OBJ_MAP}: Pixel map of object pixels (\ac{QC})\\
            	& \NEWPROD{N_LSS_STD_SKY_MAP}: Pixel map of sky pixels (\ac{QC})\\
              	& \NEWPROD{N_LSS_STD_1D}  : coadded, wavelength calibrated, collapsed 1D spectrum\\
                & \NEWPROD{STD_TRANSMISSION}: Transmission curve derived by menas of the \ac{STD} star (\ac{QC})\\
                & \NEWPROD{MASTER_N_RESPONSE}: response function (optionally including the transmission)\\
...
Input data: 	& \NEWRAW{N_LSS_SCI_RAW}\\
                & \NEWEXTCALIB{PERSISTENCE_MAP}  \\
                & \NEWSTATCALIB{LINEARITY_GEO}  \\
                & \NEWPROD{GAIN_MAP_GEO}  \\
                & \NEWPROD{BADPIX_MAP_GEO}   \\
...
Output data:	& \NEWPROD{N_LSS_SCI_OBJ_MAP}: Pixel map of object pixels (\ac{QC})\\
            	& \NEWPROD{N_LSS_SCI_SKY_MAP}: Pixel map of sky pixels (\ac{QC})\\
            	& \NEWPROD{N_LSS_SCI_2D}: coadded, wavelength calibrated 2D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{N_LSS_2d_coadd_wavecal}) \\
                & \NEWPROD{N_LSS_SCI_1D}: coadded, wavelength calibrated 1D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{N_LSS_1d_coadd_wavecal}) \\
                & \NEWPROD{N_LSS_SCI_FLUX_2D}: coadded, wavelength calibrated 2D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{N_LSS_2d_coadd_wavecal}) \\
              	& \NEWPROD{N_LSS_SCI_FLUX_1D}: coadded, wavelength 1D spectrum\\
                & (\FITS{PRO_CATG}: \FITS{N_LSS_1d_coadd_wavecal}) \\
Expected accuracies: & for wavelength: 1/10th of a pixel after post-processing\\
            & (cf.~\cite{METIS-calibration_plan}, R-MET-106, METIS-167, METIS-1371)\\
            & for flux: 10\% over an atmospheric band \\
...
The telluric correction will be done with the package \texttt{molecfit}\footnote{\url{https://www.eso.org/sci/software/pipelines/molecfit/molecfit-pipe-recipes.html}}. It is realised in three individual recipes, \NEWREC{metis_N_lss_mf_model}, which calculates the best-fit model, \NEWREC{metis_N_lss_mf_calctrans}, which creates a synthetic transmission curve, and \NEWREC{metis_N_lss_mf_correct}, which performs the actual telluric correction by means of the synthetic transmission.
...
\begin{recipedef}
Name:		& \NEWREC{metis_N_lss_mf_model}\\
...
Templates:           & None\\
Input data: 	& \NEWPROD{N_LSS_SCI_FLUX_1D}\\
...
Output data:	& \NEWPROD{MF_BEST_FIT_TAB}\\
