...
\begin{sidewaystable}
  ...
  \begin{center}
    ...
    \TPL{TELL,STD} & \CODE{DPR.TYPE==FLUX,STD}   &			   & Exposure time & \hyperref[dataitem:atmlinecat]{\EXTCALIB{ATM_LINE_CAT}} & response function\\
    		...
    		& & & & \hyperref[dataitem:]{\EXTCALIB{L}} &\\    
    		& & & & \hyperref[dataitem:]{\EXTCALIB{}} &\\    
...
\begin{itemize}
    ...
    \item \hyperref[dataitem:aopsfmodel]{\CALIB{AO_PSF_MODEL}}: For the determination of \ac{AO}-induced slit losses we intend to use a static \ac{PSF} model, which is scaled by the \ac{AO} telemetry data. Details on that are TBD.
