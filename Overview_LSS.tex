...
\begin{itemize}
    ...
    \item \textbf{Basic steps}: The basic steps aim for correcting the detector influence, in particular the dark correction and the determination of the master \ac{RSRF}. A special case is the persistence correction, which is (optionally) done as first step in the individual recipes. The basic function for that will be provided by \ac{ESO}\footnote{c.f. \ac{ELT}} and is not a dedicated recipe. 
