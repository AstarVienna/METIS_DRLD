...
Due to the complexity of \mf a large set of parameters are required. We refer to App.~B in \cite{molecfit} for more details. From the technical point-of-view, these parameters should have decent default values (which are to be determined), but changeable by the user, and then passed over to \mf via the recipe.
%\textbf{Comment by WK:} \textit{The user params given in the xshooter manual don't see really useful for me - I won't work out that in detail now as \mf will maybe change significantly in the future.}
