...
Due to the complexity of \mf a large set of parameters are required. We refer to App.~B in \cite{molecfit} for more details on the parameters for \texttt{XXX\_mf\_model} and the data types necessary to run \mf. However, not all of them are to be changed by the user. From the technical point-of-view, the majority of these parameters should have decent default values (which are to be determined beforehand). Which of these are changeable by the user is to be checked.\\
%---------------------------------------------------------------------
