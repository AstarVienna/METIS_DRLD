...
using the distortion table from \NEWREC{metis_ifu_distortion}
%if necessary).
...
\begin{recipedef}
  ...
  Input data:          & \NEWRAW{IFU_WAVE_RAW}                    \\
                       & \NEWPROD{MASTER_DARK_IFU}             \\
                       & \NEWPROD{BADPIX_MAP_IFU}               \\
                       & \NEWPROD{IFU_DISTORTION_TABLE}   \\
  Parameters:          & None                                                                    \\
  ...
  Output data:         & \NEWPROD{IFU_WAVECAL}                     \\
...
division as \NEWPROD{MASTER_FLAT_IFU} and the one-dimensional reponse
function obtained by averaging at constant wavelength as
\NEWPROD{RSRF_IFU}. The bad pixel mask can be updated by identifying pixels
that deviate strongly from their neighbours.

\begin{recipedef}
Name:                & \NEWREC{metis_ifu_rsrf}                                                     \\
...
Input data:          & \NEWRAW{IFU_RSRF_RAW} (Raw flats taken with black-body calibration lamp.)   \\
                     & \NEWPROD{MASTER_DARK_IFU}              \\
                     & \NEWPROD{BADPIX_MAP_IFU}                                                    \\
                     & \NEWPROD{IFU_WAVECAL}: image with wavelength at each pixel.                 \\
Parameters:          & None                                                                     \\
...
Output data:         & \NEWPROD{MASTER_FLAT_IFU}                                                   \\
                     & \NEWPROD{RSRF_IFU}                                                          \\
                     & \NEWPROD{BADPIX_MAP_IFU}                                                    \\
...
\begin{recipedef}
  Name:                & \NEWREC{metis_ifu_std_process}                                            \\
  ...
  Input data:          & \NEWRAW{IFU_STD_RAW} (Raw spectra of flux standard star)                  \\
                       & \NEWPROD{MASTER_DARK_IFU}            \\
                       & \NEWPROD{RSRF_IFU} (2D relative spectral response function)               \\
                       & \NEWPROD{BADPIX_MAP_IFU}  \\
                       & \NEWPROD{IFU_WAVECAL} \\
                       & \NEWPROD{IFU_DISTORTION_TABLE} \\
  Parameters:          & None                                                                   \\
  ...
  Output data:         & \NEWPROD{IFU_STD_REDUCED_CUBE}  \\
                       & \NEWPROD{IFU_STD_BACKGROUND_CUBE}                                         \\
                       & \NEWPROD{IFU_STD_REDUCED_1D}                                              \\
                       & \NEWPROD{IFU_STD_TELLURIC_1D}                                             \\
                       & \NEWPROD{FLUXCAL_TAB}                                                     \\
...
\begin{figure}[hb]
  ...
  \caption[Recipe: \REC{metis_ifu_std_process}]{%
    \NEWREC{metis_ifu_std_process} -- reduction of IFU flux standard
...
%  in the present recipe \NEWREC{metis_ifu_sci_process} or is postponed to
%  the more general recipe \NEWREC{metis_ifu_sci_postprocess} is TBD. It
...
\begin{recipedef}
Name:                & \NEWREC{metis_ifu_sci_process}                                                              \\
...
Input data:          & \NEWRAW{IFU_SCI_RAW} (Dithered science exposures.) \\
...
% TODO: I believe this should not be just FLUXCAL_TAB as it is not a table but an image
                     & \NEWPROD{FLUXCAL_TAB} (Flux calibration table) \\
                       & \NEWPROD{IFU_DISTORTION_TABLE} \\
                     & \NEWSTATCALIB{LSF_KERNEL} (Line spread kernel to be used with \CODE{molecfit}) \\
...
Output data:         & \NEWPROD{IFU_SCI_REDUCED} (2D, per exposure)           \\
                     & \NEWPROD{IFU_SCI_REDUCED_TAC} (2D, per exposure)   \\
                     & \NEWPROD{IFU_SCI_BACKGROUND} (2D, per exposure)     \\
                     & \NEWPROD{IFU_SCI_REDUCED_CUBE} (3D, per exposure) \\
                     & \NEWPROD{IFU_SCI_REDUCED_CUBE_TAC} (3D, per exposure) \\
                     & \NEWPROD{IFU_SCI_COMBINED} (3D)                       \\
                     & \NEWPROD{IFU_SCI_COMBINED_TAC} (3D)               \\
                     & \NEWPROD{IFU_SCI_OBJECT_1D}  (1D)                    \\
                     & \NEWPROD{IFU_SCI_TELLURIC_1D}                      \\
...
\begin{figure}[hb]
  \centering
    \def \globalscale {0.700000}
    \fontsize{10}{12}\selectfont
    \input{tikz/metis_ifu_sci_process}
  \caption[Recipe: \REC{metis_ifu_sci_process}]{%
    \NEWREC{metis_ifu_sci_process} -- reduction of IFU science frames.}
...
\NEWREC{metis_ifu_sci_process}. In an interactive environment it may be
...
(\NEWPROD{IFU_SCI_COMBINED}) but may be applied to other products of
\NEWREC{metis_ifu_sci_process} specified in the input set of frames.

\begin{recipedef}
  Name:                & \NEWREC{metis_ifu_tellcorr}                                                        \\
  Purpose:             & Remove telluric absorption features                                             \\
  Requirements:        & \REQ{METIS-6091}                                                                \\
  Type:                & Calibration / post processing                                                   \\
  Templates:           & ---                                                                             \\
  Input data:          & \NEWPROD{IFU_SCI_COMBINED} -- reduced combined IFU cube                            \\
                       & \NEWSTATCALIB{LSF_KERNEL} -- Line spread kernel to be used with \CODE{molecfit}         \\
                       & \NEWEXTCALIB{ATM_PROFILE} -- Atmospheric input profile to be used with \CODE{molecfit} \\
  ...
  Output data:         & \NEWPROD{IFU_SCI_REDUCED_TAC}                                                      \\
...
\begin{recipedef}
  ...
  Type:           & Science                                                                    \\
  Templates:      & None                                                                       \\
  Input data:     & Reduced science cubes (\NEWPROD{IFU_SCI_REDUCED}, \NEWPROD{IFU_SCI_REDUCED_TAC}) \\
  Parameters:     & None                                                                       \\
  ...
  Output data:    & \NEWPROD{IFU_SCI_COADD}                    \\
                  & \NEWPROD{IFU_SCI_COADD_ERROR}        \\
  QC1 parameters: & ---                                                                        \\
\end{recipedef}
...
\begin{figure}[hb]
  ...
  \caption[Recipe: \REC{metis_ifu_sci_postprocess}]{%
    \NEWREC{metis_ifu_sci_postprocess} -- post-processing (coaddition) of
...
\begin{recipedef}
  Name:                & \NEWREC{metis_ifu_distortion}                                                  \\
  ...
  Input data:          & \NEWRAW{IFU_DISTORTION_RAW} (Images of multi-pinhole mask.) \\
  Parameters:          & None                                                                        \\
  Algorithm:           & Calculate table mapping pixel position to position on sky.                  \\
  Output data:         & \NEWPROD{IFU_DISTORTION_TABLE}                                                 \\
                       & \NEWPROD{IFU_DIST_REDUCED}                                                     \\
...
\begin{figure}[hb]
  ...
  \caption[Recipe: \REC{metis_ifu_distortion}]{%
    \NEWREC{metis_ifu_distortion} -- IFU distortion calibration}
