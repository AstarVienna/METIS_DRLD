...
The algorithm described here is inspired by the spectro-perfectionism of Bolton \& Schlegel (2010, \cite{Bolton-Schlegel2010}).
...
which returns the array in the sequence that it is stored in computer memory.}
...
the model $\vec{m}$ is constructed to be sampled on square pixels $8.2\times 8.2\,\mathrm{mas^{2}}$
...
\begin{equation}
    \mat{A}_{ij} = f(\delta x_{ij}; w_d, w_m) f(\delta y_{ij}; h_d, h_m),
    \label{eq:model_matrix_element}
\end{equation}
where $w_{d}$, $h_{d}$, $w_{m}$ and $h_{m}$ are the width and height of the pixels in the data and model grids, respectively.
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \quad 0,
    & \text{if}\quad\abs{\delta} \geq \frac{D_{d} + D_{m}}{2}
  \end{cases}
\end{equation}
...
in the case where the data and model pixels grid are not aligned, a more complex algorithm must be used.

%We do not consider here the case where the data and model pixel grids are not aligned. This would be necessary if there was significant field rotation during the observing sequence.
