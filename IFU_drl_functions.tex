...
\subsubsection{IFU wavelength calibration} \label{drl:ifu_wavecal}
    \begin{recipedef}
        ...
        Outputs: &  \texttt{hdrl\_image *input} (\PROD{IFU_WAVECAL})\\
...
\subsubsection{IFU multi-exposure resampling}\label{drl:ifu_resampling}
    \begin{recipedef}
        ...
        %QC outputs: & None\\
        %Output FITS files: & None \\
        Outputs:    & \texttt{hdrl\_image *output} (\PROD{IFU_SCI_COMBINED}) \\
                    & \texttt{cpl\_error\_code} \\
...
\subsubsection{IFU multi-exposure co-add}\label{drl:ifu_coadd}
    \begin{recipedef}
        ...
        %QC outputs: & None\\
        %Output FITS files: & None \\
        Outputs:    & \texttt{hdrl\_image *output} (\PROD{IFU_SCI_COADD}) \\
%                   & \texttt{hdrl\_image *output} (\PROD{IFU_SCI_COADD_ERROR}) \\
                    & \texttt{cpl\_error\_code} \\
