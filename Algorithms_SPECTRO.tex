...
As default mode for the telluric correction \texttt{molecfit} is foreseen for all spectroscopic modes. For the \ac{IFU} we assume no major issues, especially as in highly resolved spectra individual line shapes can be fit very well. However, it is to be tested during commissioning, how well that approach works in case of the \ac{LSS} modes, as only blends of lines are visible in the low-resolution regime. A first investigation (cf. Section~\ref{sec:mf_lss_applicability}) reveals that \texttt{molecfit} is applicable to the \ac{LSS} mode as well. Nevertheless, we also include the well-proven classical method by means of a standard star.\\
...
%------------------------------------------------------------------------------
\subsubsection{Description of the telluric correction recipes provided by \texttt{telluriccorr}}\label{sec:telluriccorr_rec_desc}
The outcome of the \ac{ELT} working group meeting of 2021-03-15 was that future instrument pipelines should include dedicated recipes based on the telluric correction \texttt{telluriccorr} library~\cite{telluriccorr}. This package is based on \texttt{molecfit} and will be provided and maintained by \ac{ESO}. The telluric correction will be performed for all the LSS and IFU modes in the same way in three dedicated steps/recipes\footnote{closely following the approach as implemented in the \ac{KMOS} pipeline} as post-correction: 
\begin{itemize}
    \item Modelling/fitting of atmospheric absorption features in well defined wavelength ranges (recipes \REC{*_mf_model})
    \item Calculation of the entire transmission curve (recipes \REC{*_mf_calctrans})
    \item Application of the telluric correction (recipes \REC{*_mf_correct})
\end{itemize}
%The three steps comprise the fit of the telluric features (e.g. \REC{metis_lm_lss_mf_model} for the LM range), the calculation of the transmission curve (e.g. \REC{metis_lm_lss_mf_calctrans}) and the application of the actual correction (e.g. \REC{metis_lm_lss_mf_correct}). 
%The IFU recipe \REC{metis_ifu_telluric} comprises all three steps. 
On basis of simulations and the commissioning data we will establish a parameter set providing a good starting point for each setup. E.g. for the critical determination of the \ac{LSF} we primarily rely on the possibilities as offered by the \texttt{telluricorr}/\texttt{molecfit} package, which is based on a fitting of the \ac{LSF} by a combination of a boxcar, Gaussian or Lorentzian. We also intend to enable the user to include a dedicated line kernel instead, in case a reliable kernel model can be determined with other (external) tools (e.g. a model-based convolution of an internal \ac{LSF}, the slit-widths and/or an \ac{AO} component). In addition, also external supplementary meteorological data (e.g. line-of-sight profiles of water vapour measured by an \ac{LHATPRO} radiometer) can be included if the \texttt{telluricorr}/\texttt{molecfit} and/or \texttt{skycorr} package offers that possibility. \\
...
\begin{itemize}
    \item "classical" approach (Path \#1): The transmission of the Earth's atmosphere is determined in the classical way with the help of a \ac{STD} as described above (right branch in Fig.~\ref{Fig:tellcorrmethods}). In this case, a combined absolute flux calibration / telluric correction is applied.
...
\end{itemize}
As mentioned above, \textit{molecfit-on-science} will be the default method. It is therefore the users sole responsibility to decide whether a dedicated telluric \ac{STD} is required to observe. However, the selection of the \ac{STD} should not be arbitrary, but only possible from a provided catalogue.\\
...
\subsubsection{Investigation on the applicability of \texttt{molecfit} for the METIS long-slit mode}\label{sec:mf_lss_applicability}
