...
%------------------------------------------------------------------------------
\subsubsection{Estimate for the quality of the telluric correction (\ac{QC})}\label{sssec:tellcorrqc}
To estimate the quality of the telluric correction we follow the apporach incorporated with \texttt{molecfit} \textit{(***add Ref to new mf version doc***)}:
\begin{itemize}
    ...
    \item Determine the mean absolute difference of the resulting spectrum, excluding NaN values on the user
provided spectral ranges (e.g., only spectral ranges with telluric features).
\end{itemize}
