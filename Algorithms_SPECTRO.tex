...
In the \ac{MIR} regime it is possible to use a standard star for both, the telluric correction and the absolute flux calibration, if a good model containing intrinsic spectral features as well as absolute fluxes of the individual star is available. In that way, telluric correction and flux calibration can be applied simultaneously, if the standard star is observed directly before/after the science target at the same airmass (ideally in the same direction) assuming the atmosphere has not changed significantly in the meantime. The current calibration plan foresees obervations of up to two standard stars per science \ac{OB} (cf. \cite{METIS-calibration_plan}, Section 3.8) for \ac{OB}s requiring an accurate flux calibration. \\
%------------------------------------------------------------------------------
