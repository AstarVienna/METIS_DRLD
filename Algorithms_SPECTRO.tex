...
\subsubsection{Investigation on the applicability of \texttt{molecfit} for the \ac{LSS} mode}
One of the most critical steps in the framework of \texttt{molecfit} is the fit of the \ac{LSF}. Especially for low-resolving spectrographs this issue might become tricky as isolated lines for the line shape fitting become rare due to blending effects. In order to get a first estimate we simulated \ac{LSS} observations of a point source with a constant (i.e. flat) spectrum with ScopeSIM\footnote{\url{https://scopesim.readthedocs.io/en/stable/}\newline\url{https://github.com/AstarVienna/ScopeSim}} in a way to get spectra with an unaffected \ac{LSF}, i.e. the pure spectra as seen on the detector. We used the ScopeSIM internal functionality to rectify the orders, extracted the object spectrum, and removed the background. The resulting 1d spectra were used as input for \texttt{molecfit v1.5.9} (see Fig.~\ref{Fig:lss_sim}). We focused on the M and N bands using the fitting regions and molecules given in Fig.~\ref{Fig:lss_sim}. The fitting results in Fig.~\ref{Fig:lss_results} show good coincidence between the input spectra and the fits, leading to a fairly good telluric correction (cf. Fig.~\ref{Fig:lss_corr}). \\
We emphasize that these results are based on first attempts only. The quality of the correction can most probably be improved and therefore should be seen as demonstration that in principle \texttt{molecfit} can be considered as an option for the telluric correction. A more detailed investigation will be done when real data are available.
\begin{figure}[ht]
  \centering
